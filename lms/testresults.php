...
<script type="text/javascript">
 ...
 else
 {
    ...
    $test = mysqli_query($mysqli,"/*" . MYSQLND_QC_ENABLE_SWITCH . "*/" . "SELECT name, testfor FROM testgroups WHERE id='".$member['testid']."' LIMIT 1;");
    $testname = mysqli_fetch_array($test);
    echo "<td align='center'>";
    if ($testname['testfor']=='expert') { 
     echo "<i class='fa fa-user-md fa-lg' title='Тест для экспертов'></i> "; 
    } else 
    if ($testname['testfor']=='member') {
     echo "<i class='fa fa-user fa-lg' title='Тест для участников'></i> "; }
    else {
     echo "<i class='fa fa-slideshare fa-lg' title='Пробный тест'></i>"; 
    }
